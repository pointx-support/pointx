...
export function compareAccumulators(a, b, tieBreakOrder = ['totalPoints', 'totalKills', 'booyahs', 'highestPlacement']) {
    for (const criteria of tieBreakOrder) {
        if (criteria === 'totalPoints') {
            ...
            if (b.latestMatchPoints !== a.latestMatchPoints)
                return b.latestMatchPoints - a.latestMatchPoints;
        }
    }
 ...
 */
export function calculateTournamentStandings(tournament, options) {
    const { teams, matches, scoringPreset } = tournament;
    const includeDrafts = options?.includeDrafts ?? false;
    // Filter eligible matches
    const eligibleMatches = matches
        .filter((m) => {
        if (!includeDrafts && m.status === 'Draft')
            return false;
        if (options?.matchRange?.start && m.matchNumber < options.matchRange.start)
            return false;
        if (options?.matchRange?.end && m.matchNumber > options.matchRange.end)
            return false;
        return true;
    })
    ...
    const sortedCurrent = Array.from(currentAccumulators.values()).sort((a, b) => compareAccumulators(a, b, tieBreakOrder));
    ...
    if (eligibleMatches.length > 1) {
        ...
        const sortedPrev = Array.from(prevAccumulators.values()).sort((a, b) => compareAccumulators(a, b, tieBreakOrder));
    ...
    return sortedCurrent.map((item, index) => {
        ...
        const avgPointsPerMatch = item.matchesPlayed > 0 ? Number((item.totalPoints / item.matchesPlayed).toFixed(1)) : 0;
        const avgKillsPerMatch = item.matchesPlayed > 0 ? Number((item.totalKills / item.matchesPlayed).toFixed(1)) : 0;
        return {
            rank: currentRank,
            previousRank,
            rankDelta,
 ...
 */
export function calculateTopFraggers(tournament, options) {
 ...
 */
export function calculateTournamentSummary(tournament) {
    ...
    let highestSingleMatchScore;
    let highestSingleMatchKills;
